...
export default {
  ...
  methods: {
    ...
    initParmes(tab, stockDateSelect, myBrandSelect, myCarSelect) {
      ...
      if (tab == 1) { //1按组织 2按品牌 3按车系
        ...
        delete parmes.orgId; // 删除orgId===========================
        delete parmes.brandCode; // 删除brandCode
        delete parmes.seriesCode; // 删除seriesCode
        parmes.orgType = '3'; //====================//1 集团 2大区 3 小区 4 4s店
        parmes.orgCode = this.$route.query.orgCode; //========================
  ...
  computed: {
    ...
    ...mapState({
      ...
      STOCK_ZAIKU_AREA_LITTLE: state => state.STOCK_ZAIKU_AREA_LITTLE,
      STOCK_ZAITU_AREA_LITTLE: state => state.STOCK_ZAITU_AREA_LITTLE,
      STOCK_KEGONGXIAO_AREA_LITTLE: state => state.STOCK_KEGONGXIAO_AREA_LITTLE,
      STOCK_ZAIKUJIN_AREA_LITTLE: state => state.STOCK_ZAIKUJIN_AREA_LITTLE,
